...
CC = gcc
LD = ld
LIB = -I lib/ -I fs/ -I lib/kernel/ -I lib/user/ -I kernel/ -I device/ -I thread/ -I userprog/ -I shell/
ASFLAGS = -f elf
...
$(BUILD_DIR)/ide.o: device/ide.c device/ide.h lib/stdint.h kernel/interrupt.h \
		kernel/global.h lib/stdio.h lib/string.h userprog/syscall-init.h kernel/memory.h \
		device/timer.h lib/kernel/list.h lib/kernel/stdio-kernel.h fs/super_block.h
	$(CC) $(CFLAGS) $< -o $@

$(BUILD_DIR)/fs.o: fs/fs.c fs/fs.h fs/dir.h fs/inode.h fs/super_block.h \
		device/ide.h kernel/global.h lib/kernel/bitmap.h lib/stdint.h
	$(CC) $(CFLAGS) $< -o $@
...
$(BUILD_DIR)/fork.o: userprog/fork.c userprog/fork.h lib/stdint.h thread/thread.h \
		lib/kernel/stdio-kernel.h kernel/debug.h kernel/interrupt.h \
		kernel/global.h lib/kernel/bitmap.h kernel/memory.h fs/file.h \
		userprog/process.h lib/string.h lib/kernel/list.h
	$(CC) $(CFLAGS) $< -o $@

$(BUILD_DIR)/shell.o: shell/shell.c shell/shell.h lib/stdint.h thread/thread.h \
		kernel/debug.h kernel/global.h fs/file.h shell/buildin_cmd.h
	$(CC) $(CFLAGS) $< -o $@

$(BUILD_DIR)/buildin_cmd.o: shell/buildin_cmd.c shell/buildin_cmd.h lib/stdint.h\
		kernel/debug.h kernel/global.h fs/file.h fs/dir.h fs/file.h
	$(CC) $(CFLAGS) $< -o $@

$(BUILD_DIR)/exec.o: userprog/exec.c userprog/exec.h lib/stdint.h\
		kernel/debug.h kernel/global.h fs/fs.h kernel/memory.h
	$(CC) $(CFLAGS) $< -o $@
...
hd:
	sudo dd if=$(BUILD_DIR)/kernel.bin of=/home/yuyu/os/code/a/hd60M.img bs=512 count=200 seek=9 conv=notrunc
